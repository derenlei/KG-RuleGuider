...
class GraphSearchPolicy(nn.Module):
    ...
    def transit(self, e, obs, kg, use_action_space_bucketing=True, merge_aspace_batching_outcome=False):
        """
        Compute the next entity distribution based on
            (a) the current node (entity) in KG and the query relation
            (b) entity history representation
        ...
        :param merge_aspace_batch_outcome: If set, merge the transition probability distribution
            generated of different entity space bucket into a single batch.
        :return
            With aspace batching and without merging the outcomes:
                db_outcomes: (Dynamic Batch) (action_space, action_dist)
                    ...
                    action_dist: (Batch) distribution over entities.
                inv_offset: Indices to set the dynamic batching output back to the original order.
                entropy: (Batch) entropy of entity distribution.
            Else:
                action_dist: (Batch) distribution over entities.
                entropy: (Batch) entropy of entity distribution.
        """
        ...
        def policy_nn_fun(X2, action_space):
            ...
            A = kg.get_entity_embeddings(e_space)
    ...
    def update_path(self, action, kg, offset=None):
        ...
        entity_embedding = kg.get_entity_embeddings(action[1])
    ...
    def define_modules(self):
        ...
        self.W1 = nn.Linear(input_dim, self.entity_dim)
        ...
        if torch.cuda.is_available():
            ...
            self.W1Dropout = self.W1Dropout.cuda()
            self.W2Dropout = self.W1Dropout.cuda()
            self.LN1Dropout = self.LN1Dropout.cuda()
            self.LN2Dropout = self.LN2Dropout.cuda()
    ...
    def initialize_modules(self):
        if self.xavier_initialization:
            ...
            for name, param in self.entity_agent.named_parameters():
                if 'bias' in name:
