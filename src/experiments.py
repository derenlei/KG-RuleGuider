...
def initialize_model_directory(args, random_seed=None, initialize=False):
    if not initialize and args.model_dir != os.path.join(os.path.dirname(os.path.dirname(__file__)), 'model'):
...
def inference(lf):
    ...
    if args.compute_map:
        ...
        pred_scores, _ = lf.forward(dev_data, verbose=args.save_beam_search_paths)
        ...
        pred_scores, _ = lf.forward(test_data, verbose=False)
